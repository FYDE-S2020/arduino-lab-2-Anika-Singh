...
char auth[] = "l8g4gtvlfqT8KCQx84CwtmGaTpCiAFvU";
...
BLYNK_WRITE(V2)
{
    ...
    if (LED_ == 1){
      dutycyle = val;
      ledcWrite(ledChannel, dutycyle); // Note that this takes ledChannel as an argument, NOT the pin! Set duty = val.
  }
}
   ...
   // Write the arduino uptime every second
    if (time_count == 100){
        ...
        while(Serial.available()) { // Check if serial is available every 10 ms
            ...
            messages.concat(character);
        }
        if (messages != "") {
            Blynk.virtualWrite(V2, messages);
